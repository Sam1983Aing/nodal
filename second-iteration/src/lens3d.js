...
/* Interface amber, matching --accent-rgb on the black plate. The drawing and
   the engraved marks are interface and follow it. The RAYS deliberately do
   not: light is not a brand colour, so it runs cool white to warm, which is
   also what the wavelength-split trace will need. */
const ACCENT = new THREE.Color(0xff9a3c);
...
function irisScaleTexture(spec) {
  ...
  return engraveCanvas((ctx, W, H) => {
    ...
    stops.forEach((s, i) => {
      ...
      const wide = i === 0;
      ctx.strokeStyle = ctx.fillStyle = wide ? '#ff9a3c' : '#e8e5de';
      ctx.lineWidth = 4;
      ctx.beginPath();
      ctx.moveTo(x, H * 0.62);
      ctx.lineTo(x, H * 0.88);
